...
Replaces remote <img src="...svg"> tags with the SVG content embedded
directly. All element IDs are namespaced with the SVG filename stem to
prevent collisions when multiple SVGs are inlined in the same document.

Blank lines are stripped from the embedded SVG because CommonMark HTML blocks
(type 6) terminate at the first blank line — any blank line inside an embedded
SVG causes Marp's parser to exit the HTML block and treat subsequent lines as
Markdown, silently dropping <defs> and other SVG content.
...
IMG_PATTERN = re.compile(
    r'<img\s+src="https://raw\.githubusercontent\.com/graingert/'
    r'anyio_why_already_slides/refs/heads/default/([\w]+\.svg)"([^>]*)>'
)
